...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#btn_cari").click(function(){  
            var nama_cara_bayar=$('#filter_nama_cara_bayar').val();
            load(nama_cara_bayar); 
        });
        // click simpan 
...
<script type="text/javascript">
    // load data table
    function load(nama_cara_bayar='') {
        closeModal();
        var t_table = $('#tb_data').DataTable();
        t_table.destroy();
        t_table = $('#tb_data').DataTable( {
            "ajax": {
                "url": "<?php echo base_url('master/cara_bayar/load_json') ?>",
                "type": "POST",
                "data": {"nama_cara_bayar": nama_cara_bayar},
            },
            "language": {
    ...
    function form_edit(data_id=""){
        ...
        $.ajax({// menggunakan ajax form
            url: "<?php echo base_url('master/cara_bayar/get_detail_data'); ?>",
            type: "POST",
            ...
            success: function (output) {
                ...
                $("#id_cara_bayar").val(output.data.id_cara_bayar);
                $("#nama_cara_bayar").val(output.data.nama_cara_bayar);
    ...
    function simpan() {        
        ...
        $.ajax({
            url: "<?php echo base_url('master/cara_bayar/'); ?>" + form,
            type: "POST",
            data : data_fields,
    ...
    function form_hapus(data_id=""){
        swal({
             ...
             $.ajax({// menggunakan ajax form
                url: "<?php echo base_url('master/cara_bayar/delete_process'); ?>",
                type: "POST",
                data: {
                    "id_hapus":data_id
                },
